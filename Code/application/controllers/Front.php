...
class Front extends CI_Controller
{
		...
		if ($this->ion_auth->logged_in()) {
			...
			if ($features) {
				...
				foreach ($features as $key => $feature) {
					...
					<span class="badge light badge-primary"><a href="' . base_url('front/edit-feature/' . $feature["id"]) . '" data-id="' . $feature["id"] . '" class="text-primary btn-success" data-placement="top" title="' . ($this->lang->line('edit') ? htmlspecialchars($this->lang->line('edit')) : 'Edit') . '"><i class="fas fa-pen"></i></a></span>
					<span class="badge light badge-danger ms-2"><a href="#" class="text-danger delete_feature" data-id="' . $feature["id"] . '" data-bs-toggle="tooltip"  data-placement="top"title="' . ($this->lang->line('delete') ? htmlspecialchars($this->lang->line('delete')) : 'Delete') . '"><i class="fas fa-trash"></i></a></span>
				</div>';
				}
				return print_r(json_encode($temp));
			} else {
	...
	{
		if ($this->ion_auth->logged_in() && is_saas_admin() && $this->uri->segment(3) && is_numeric($this->uri->segment(3))) {
