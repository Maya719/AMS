...
class Notifications extends CI_Controller
{
	...
	public function get_live_notifications()
	{
		if ($this->ion_auth->logged_in()) {
			...
			$data['unread_support_msg_count'] = get_unread_support_msg_count();
			...
			foreach ($notifications as $key => $notification) {
				...
				if ($notification['type'] == 'offline_request' && $this->ion_auth->in_group(3)) {
					...
					if ($task) {
						...
						$extra = ($this->lang->line('project') ? $this->lang->line('project') : 'Project') . ': <a target="_blank" href="' . base_url('projects/detail/' . $task[0]['project_id']) . '">' . $task[0]['project_title'] . '</a> ';
						$extra .= ($this->lang->line('task') ? $this->lang->line('task') : 'Task') . ': <a target="_blank" href="' . base_url('board/tasks/' . $task[0]['project_id']) . '">' . $task[0]['title'] . '</a>';
					}
				} elseif ($notification['type'] == 'task_status') {
				...
				$temp[$key]['first_name'] = $notification['first_name'] . " " . $notification['last_name'];
				$temp[$key]['is_read'] = $notification['is_read'] == 1 ? '<div class="badge badge-success">Yes</div>' : '<div class="badge badge-danger">No</div>';
				$temp[$key]['action'] = '<div class="d-flex">
				<span class="badge light badge-primary"><a href="' . $notification_url . '" data-id="' . $notification["id"] . '" class="text-primary" title="' . ($this->lang->line('view') ? htmlspecialchars($this->lang->line('view')) : 'View') . '"><i class="fas fa-eye"></i></a></span>
				<span class="badge light badge-danger ms-2"><a href="javascript:void()" class="text-danger delete_notification" data-id="' . $notification["id"] . '" data-bs-toggle="tooltip" data-placement="top" title="' . ($this->lang->line('delete') ? htmlspecialchars($this->lang->line('delete')) : 'Delete') . '"><i class="fas fa-trash"></i></a></span>
			  </div>';
