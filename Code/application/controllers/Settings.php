...
class Settings extends CI_Controller
{
	...
	public function save_grace_minutes_setting()
	{
		if ($this->ion_auth->logged_in() && ($this->ion_auth->is_admin() || $this->ion_auth->in_group(4) || permissions('time_schedule_view'))) {
			if ($this->ion_auth->in_group(4)) {
				$setting_type = 'grace_minutes_' . $this->session->userdata('saas_id');
			} else {
				$setting_type = 'grace_minutes_' . $this->session->userdata('saas_id');
			}
			...
			if ($this->input->post('enableGraceMinutes')) {
				$grace_day = $this->input->post('days_counter');
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if ($this->settings_model->save_settings($setting_type, $data)) {
					$this->data['error'] = false;
					$this->data['data'] = $data_json;
					$this->data['message'] = $this->lang->line('company_setting_saved') ? $this->lang->line('company_setting_saved') : "Setting Saved.";
					echo json_encode($this->data);
				} else {
					$this->data['error'] = true;
					$this->data['message'] = $this->lang->line('something_wrong_try_again') ? $this->lang->line('something_wrong_try_again') : "Something wrong! Try again.";
	...
	public function save_hierarchy_setting()
	{
		if ($this->ion_auth->logged_in()) {
			...
			foreach ($data as $value) {
				...
				$recomender_approver = $value["recomender_approver"];
				$array[] = [
					'step_no' => $step_no,
					'group_id' => $group_id,
					'recomender_approver' => $recomender_approver,
					'saas_id' => $this->session->userdata('saas_id')
				];
			}
	...
	public function save_permissions_setting()
	{
		...
		if ($this->ion_auth->logged_in() && ($this->ion_auth->is_admin() || change_permissions(''))) {
			...
			foreach ($roles as $role) {
				if ($role['id'] !== $this->input->post($role['name'])) {
					$data_json = array(
						...
						'biometric_request_view_selected' => $this->input->post($role['name'] . '_biometric_request_view_selected') != '' ? 1 : 0,
						...
						'task_view_selected' => $this->input->post($role['name'] . '_task_view_selected') != '' ? 1 : 0,
						...
						'company_edit' => $this->input->post($role['name'] . '_company_edit') != '' ? 1 : 0,
					);
	...
	public function leaves_type_edit()
	{
		if ($this->ion_auth->logged_in() && !$this->ion_auth->in_group(3) && !$this->ion_auth->in_group(4)) {
			...
			$this->form_validation->set_rules('name', 'Name', 'trim|required|strip_tags|xss_clean');
			$this->form_validation->set_rules('count', 'Count', 'trim|required|strip_tags|xss_clean');
	...
	public function leaves_type_create()
	{
		if ($this->ion_auth->logged_in() && !$this->ion_auth->in_group(3) && !$this->ion_auth->in_group(4)) {
			$this->form_validation->set_rules('name', 'Name', 'trim|required|strip_tags|xss_clean');
			$this->form_validation->set_rules('duration', 'Duration', 'trim|required|strip_tags|xss_clean');
			if ($this->form_validation->run() == TRUE) {
				$data = array(
					'saas_id' => $this->session->userdata('saas_id'),
					'name' => $this->input->post('name'),
