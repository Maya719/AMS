...
class Todo extends CI_Controller
{
	...
	public function delete($id='')
	{
		if ($this->ion_auth->logged_in())
		{
			...
			if(!empty($id) && is_numeric($id) && $this->todo_model->delete($this->session->userdata('user_id'), $id)){
				...
				$this->session->set_flashdata('message', $this->lang->line('todo_deleted_successfully')?$this->lang->line('todo_deleted_successfully'):"ToDo deleted successfully.");
				$this->session->set_flashdata('message_type', 'success');

				$this->data['error'] = false;
				$this->data['message'] = $this->lang->line('todo_deleted_successfully')?$this->lang->line('todo_deleted_successfully'):"ToDo deleted successfully.";
		...
		if ($this->ion_auth->logged_in())
		{
			...
			if($this->form_validation->run() == TRUE){
				...
				if($this->todo_model->edit($this->input->post('id'), $data)){
					$this->data['error'] = false;
					$this->data['message'] = $this->lang->line('todo_updated_successfully')?$this->lang->line('todo_updated_successfully'):"ToDo updated successfully.";
					echo json_encode($this->data); 
				}else{
					$this->data['error'] = true;
					$this->data['message'] = $this->lang->line('something_wrong_try_again')?$this->lang->line('something_wrong_try_again'):"Something wrong! Try again.";
		...
		if ($this->ion_auth->logged_in())
		{
			...
			if($this->form_validation->run() == TRUE){
				...
				if($this->todo_model->edit($this->input->post('update_id'), $data)){
					$this->session->set_flashdata('message', $this->lang->line('todo_updated_successfully')?$this->lang->line('todo_updated_successfully'):"ToDo updated successfully.");
					$this->session->set_flashdata('message_type', 'success');
					$this->data['error'] = false;
					$this->data['message'] = $this->lang->line('todo_updated_successfully')?$this->lang->line('todo_updated_successfully'):"ToDo updated successfully.";
					echo json_encode($this->data); 
	...
	public function create()
	{
		if ($this->ion_auth->logged_in())
		{
			...
			if($this->form_validation->run() == TRUE){
				...
				if($ToDo_id){
					$this->session->set_flashdata('message', $this->lang->line('todo_created_successfully')?$this->lang->line('todo_created_successfully'):"ToDo created successfully.");
					$this->session->set_flashdata('message_type', 'success');
					$this->data['error'] = false;
					$this->data['message'] = $this->lang->line('todo_created_successfully')?$this->lang->line('todo_created_successfully'):"ToDo created successfully.";
					echo json_encode($this->data); 
				}else{
					$this->data['error'] = true;
					$this->data['message'] = $this->lang->line('something_wrong_try_again')?$this->lang->line('something_wrong_try_again'):"Something wrong! Try again.";
	...
	{
		if ($this->ion_auth->logged_in() && is_module_allowed('todo') && !$this->ion_auth->in_group(3) && ($this->ion_auth->is_admin() || permissions('todo_view')))
		{
