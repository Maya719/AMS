...
//-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    /**
     * The path to log file of the webhook events
     * 
     * @var string 
     */
    public $webhookLogPath = '../';
...
//-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    function validateSignature($dataArray, $secret, $signature) {
        ...
        $hash = base64_encode(hash_hmac('sha256', $data, $keySecret, true));

        if ($signature === $hash) {
            $this->logWebhook('valid signature');
            return true;
        } else {
            $this->logWebhook('Not a valid signature');
            die;
}
    }
...
//-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    /**
     * It will log the webhook events
     * 
     * @param type $msg It is the string message that will be written in the log file
     */
    function logWebhook($msg) {
        error_log(PHP_EOL . date('d.m.Y h:i:s') . ' - ' . $msg, 3, $this->webhookLogPath . 'myfatoorah_webhook.log');
    }
