...
class Projects_model extends CI_Model
{
    ...
    function get_timesheet()
    {
        ...
        if ($this->ion_auth->is_admin() || permissions('task_view_all')) {
            ...
            if (permissions('task_view_selected')) {
                if (isset($get['user_id']) && !empty($get['user_id'])) {
                    $where = " WHERE t.user_id = " . $get['user_id'];
                } else {
                    ...
                    if (!empty($selected)) {
                        ...
                        $where = " WHERE t.user_id IN ($userIdsString)";
                    }
                }
            } else {
    ...
    function get_projects_list()
    {
        ...
        foreach ($results as &$result) {
            ...
            $project_users = $this->get_project_users($result['id']);
            $result["project_users"] = $project_users;
            $result["created"] = date('d M, Y', strtotime($result['created']));
            $result["stats"] = '<i class="fas fa-layer-group"></i> ' . (get_count('id', 'tasks', 'status = 4 and project_id=' . $result['id'])) . '/' . (get_count('id', 'tasks', 'project_id=' . $result['id'])) . ' ' . ($this->lang->line('task_completed') ? $this->lang->line('task_completed') : 'Task Completed');
            $html = '<ul class="kanbanimg col-6">';
            foreach ($result["project_users"] as $user) {
                if (isset($user["profile"]) && !empty($user["profile"])) {
                    if (file_exists('assets/uploads/profiles/' . $user["profile"])) {
                        $file_upload_path = base_url('assets/uploads/profiles/' . $user["profile"]);
                    } else {
                        $file_upload_path = base_url('assets/uploads/f' . $this->session->userdata('saas_id') . '/profiles/' . $user["profile"]);
                    }
                }
                if (!empty($issue["profile"])) {
                    $html .= '
                            <li style="cursor: pointer;">
                            <img data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-content="' . ($user["first_name"] . ' ' . $user["last_name"]) . '" src="' . $file_upload_path . '" alt="' . $file_upload_path . '"></li>
                        ';
                } else {
                    $html .= '
                            <li><span data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-content="' . ($user["first_name"] . ' ' . $user["last_name"]) . '">' . (substr($user["first_name"], 0, 1) . substr($user["last_name"], 0, 1)) . '</span></li>
                        ';
                }
            }
            $html .= '</ul>';
    ...
    function get_projects($user_id = '', $project_id = '', $limit = '', $start = '', $filter_type = '', $filter = '')
    {
        ...
        if (permissions('project_view_selected')) {
            ...
            if (!empty($selected)) {
                $userIdsString = implode(',', $selected);
                $where .= (empty($where)) ? " WHERE pu.user_id IN ($userIdsString)" : " AND pu.user_id IN ($userIdsString)";
            }
        } else {
    ...
    function get_tasks($user_id = '', $task_id = '', $project_id = '', $search = '', $sort = 'created', $priority = '', $upcoming = '')
    {
        ...
        if (!empty($user_id) && is_numeric($user_id)) {
            if (is_client()) {
                ...
                $where .= (empty($where)) ? " WHERE tu.user_id=$user_id " : " AND tu.user_id=$user_id ";
            }
            ...
            if (!empty($selected)) {
                $userIdsString = implode(',', $selected);
                $where .= (empty($where)) ? " WHERE tu.user_id IN ($userIdsString)" : " AND tu.user_id IN ($userIdsString)";
            }
        }
        ...
        if (!$this->ion_auth->is_admin() && !permissions('task_view_all')) {
            if (is_client()) {
                ...
                if (permissions('task_view_selected')) {
                    ...
                    if (!empty($selected)) {
                        $userIdsString = implode(',', $selected);
                        $where = " WHERE tu.user_id IN ($userIdsString)";
                    }
                } else {
                    if (empty($get['user'])) {
