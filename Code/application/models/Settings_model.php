...
class Settings_model extends CI_Model
{
    ...
    {
        $data = 'grace_minutes_' . $this->session->userdata('saas_id');
        $query = $this->db->query("SELECT value FROM settings WHERE type = '$data'");
        $result = $query->result_array();
    ...
    function get_roles()
    {
        ...
        foreach ($results as $result) {
            ...
            if ($result["saas_id"] == $saas_id) {
                ...
                $changePermissions = json_decode($result["change_permissions_of"]);
                foreach ($changePermissions as $changePermission) {
                    $query = $this->db->query("SELECT * FROM groups WHERE id=" . $changePermission);
                    $permission = $query->result_array();
                    $group_name = htmlspecialchars($permission[0]['description']);
                    if (!empty($group_name)) {
                        $tempRow['change_permissions'] .= '<span>' . $group_name . '</span>' . '<br>';
                    }
                }
    ...
    function save_hierarchy_setting($data)
    {
        $this->db->where('saas_id', $this->session->userdata('saas_id'));
        $this->db->delete('leave_hierarchy');
        if ($this->db->insert_batch('leave_hierarchy', $data)) {
            return true;
        } else {
            return false;
        }
    ...
    function get_leaves_type()
    {
        ...
        foreach ($results as $result) {
            if ($result['duration'] == 'year') {
                $tempRow['duration'] = 'Annually';
            } elseif ($result['duration'] == '3_months') {
                $tempRow['duration'] = 'For 3 Months';
            } elseif ($result['duration'] == '4_months') {
                $tempRow['duration'] = 'For 4 Months';
            } elseif ($result['duration'] == '6_months') {
                $tempRow['duration'] = 'For 6 Months';
            }
            $tempRow['name'] = $result['name'];
