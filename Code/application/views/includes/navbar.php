...
<div class="main-sidebar sidebar-style-2">
  <aside id="sidebar-wrapper">
    ...
    <ul class="sidebar-menu">
      <li <?= (current_url() == base_url('/') || current_url() == base_url('home'))?'class="active"':''; ?>><a class="nav-link" href="<?=base_url()?>"><i class="fas fa-home text-secondary"></i> <span><?=$this->lang->line('dashboard')?$this->lang->line('dashboard'):'Dashboard'?></span></a></li>
     ...
     <?php if(($this->ion_auth->is_admin() || permissions('attendance_view_all') || permissions('attendance_view_selected') || permissions('attendance_view') || permissions('leaves_view') || permissions('biometric_request_view') || permissions('plan_holiday_view')) && !is_saas_admin() && (is_module_allowed('leaves') || is_module_allowed('attendance') || is_module_allowed('biometric_missing'))){ ?>           
        ...
        <span><?=$this->lang->line('ams')?$this->lang->line('ams'):'AMS'?></span></a>
          <ul class="dropdown-menu">
            ...
            <?php if(($this->ion_auth->is_admin() || permissions('attendance_view_all') || permissions('attendance_view_selected')) && is_module_allowed('attendance')) { ?>
              <li <?=(current_url() == base_url('attendance') || $this->uri->segment(1) == 'attendance' )?'class="active"':''; ?>><a class="nav-link" href="<?=base_url('attendance')?>"><span><?=$this->lang->line('attendance')?$this->lang->line('attendance'):'Attendance'?></span></a></li>
            <?php } ?>
            ...
            <?php if(is_module_allowed('attendance') && permissions('attendance_view') && !permissions('attendance_view_all') && !permissions('attendance_view_selected')){ ?>
              <li <?=(current_url() == base_url('attendance/user_attendance') || $this->uri->segment(1) == 'attendance' )?'class="active"':''; ?>><a class="nav-link" href="<?=base_url('attendance/user_attendance')?>"><span><?=$this->lang->line('attendance')?$this->lang->line('attendance'):'Attendance'?></span></a></li>
            <?php } ?>
      ...
      <?php if(($this->ion_auth->is_admin() || is_client() || permissions('project_view') || permissions('task_view') || permissions('gantt_view') || permissions('calendar_view')) && !is_saas_admin() && (is_module_allowed('projects') || is_module_allowed('tasks') || is_module_allowed('timesheet') || is_module_allowed('gantt') || is_module_allowed('calendar'))){ ?>           
        <li class="dropdown <?=((current_url() == base_url('projects') || $this->uri->segment(1) == 'projects' || $this->uri->segment(2) == 'list') || (current_url() == base_url('projects/tasks') || $this->uri->segment(2) == 'tasks-list') || current_url() == base_url('projects/timesheet') || current_url() == base_url('projects/gantt') || current_url() == base_url('projects/calendar') )?'active':''; ?>">

        <a class="nav-link has-dropdown" href="#"><i class="fas fa-project-diagram text-info"></i>
        <span><?=$this->lang->line('pms')?$this->lang->line('pms'):'PMS'?></span></a>
          <ul class="dropdown-menu">
            <?php if(is_module_allowed('projects') && ($this->ion_auth->is_admin() || permissions('project_view'))){ ?>
              <li <?=(current_url() == base_url('projects/list') || $this->uri->segment($this->uri->total_segments()) == 'projects' || $this->uri->segment(2) == 'list')?'class="active"':''; ?>><a class="nav-link" href="<?=base_url('projects/list')?>"><span><?=$this->lang->line('projects')?$this->lang->line('projects'):'Projects'?></span></a></li>
            <?php } ?>

            <?php if(is_module_allowed('tasks') && ($this->ion_auth->is_admin() || permissions('task_view'))){ ?>
              <li <?=(current_url() == base_url('projects/tasks') || $this->uri->segment(2) == 'tasks-list' || $this->uri->segment(2) == 'tasks')?'class="active"':''; ?>><a class="nav-link" href="<?=base_url('projects/tasks')?>"> <span><?=$this->lang->line('tasks')?$this->lang->line('tasks'):'Tasks'?></span></a></li>
            <?php } ?>
      ...
      <?php if(($this->ion_auth->is_admin() || permissions('user_view')) && !is_saas_admin() && is_module_allowed('team_members') ){ ?>
        <li <?=(current_url() == base_url('users') || $this->uri->segment($this->uri->total_segments()) == 'users')?'class="active"':''; ?>><a class="nav-link" href="<?=base_url('users')?>"><i class="fas fa-users text-warning"></i><span><?=$this->lang->line('employees')?$this->lang->line('employees'):'Employees'?></span></a></li>
      <?php } ?>
      ...
      <?php if (($this->ion_auth->is_admin() || permissions('todo_view')) && !is_saas_admin() && is_module_allowed('todo')){ ?>  
        <li <?= (current_url() == base_url('todo'))?'class="active"':''; ?>><a class="nav-link" href="<?=base_url('todo')?>"><i class="fas fa-clipboard-check text-warning"></i> <span><?=$this->lang->line('to_do')?$this->lang->line('to_do'):'Todo'?></span></a></li>
      <?php } ?>
      ...
      <?php if ($this->ion_auth->is_admin()){ ?>  
        <li <?= (current_url() == base_url('plans'))?'class="active"':''; ?>><a class="nav-link" href="<?=base_url('plans')?>"><i class="fas fa-dollar-sign"></i> <span><?=$this->lang->line('subscription_plans')?$this->lang->line('subscription_plans'):'Plans'?></span></a></li>
      <?php } ?>
      ...
      <?php if(($this->ion_auth->is_admin() || permissions('reports_view')) && is_module_allowed('reports')){ ?>           
        ...
        <a class="nav-link has-dropdown" href="#"><i class="fas fa-chart-bar text-danger"></i> 
        <span><?=$this->lang->line('reports')?$this->lang->line('reports'):'Reports'?></span></a>
          <ul class="dropdown-menu">
              ...
              <?php if($this->ion_auth->is_admin() || permissions('user_view_all') || permissions('user_view_selected')){ ?>
        ...
        <span><?=$this->lang->line('settings')?$this->lang->line('settings'):'Settings'?></span></a>
          <ul class="dropdown-menu">
            ...
            <?php }else{ ?>
              ...
              <?php if($this->ion_auth->is_admin() || permissions('leave_type_view')){ ?>
                <li <?=(current_url() == base_url('settings/hierarchy'))?'class="active"':''; ?>><a class="nav-link" href="<?=base_url('settings/hierarchy')?>"><?=$this->lang->line('hierarchy')?$this->lang->line('hierarchy'):'Leave hierarchy'?></a></li>
              <?php } ?>
              ...
              <?php if($this->ion_auth->is_admin() || permissions('time_schedule_view')){ ?>
                <li <?=(current_url() == base_url('settings/department'))?'class="active"':''; ?>><a class="nav-link" href="<?=base_url('settings/department')?>"><?=$this->lang->line('time_schedule')?$this->lang->line('time_schedule'):'Applied Policy'?></a></li>
              <?php } ?>
