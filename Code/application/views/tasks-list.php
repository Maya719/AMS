        ...
        <section class="section">
          <div class="section-header">
            ...
            <?=$this->lang->line('tasks')?$this->lang->line('tasks'):'Tasks'?> 
              <?php
              if(my_plan_features('tasks')){ if ($this->ion_auth->is_admin() || permissions('task_view_all') || permissions('task_create')){ ?>
          ...
          <div class="section-body">
            <div class="row" id="tool">
              ...
              <div class="form-group col-md-<?=($this->ion_auth->is_admin() || permissions('task_view_all') || permissions('task_view_selected'))?'3':'6'?>">
          ...
          <div class="tab-content tab-bordered" id="myTab3Content">
            ...
            <div class="tab-pane fade" id="timesheet" role="tabpanel" aria-labelledby="timesheet-tab">
              <table class='table-striped' id='timesheet_list'
                ...
                <thead>
                  <tr>
                    <?php if($this->ion_auth->is_admin() || permissions('task_view_all')){ ?>
...
<script>
$('#modal-edit-task').click(function (event) {
  event.preventDefault();
  var taskId = $('#update_id').val();

  var data = {
    "task_id": taskId
  };
  $.ajax({
        url: '<?= base_url('projects/get_task_user') ?>',
        method: "POST",
        data: data,
        success: function (response) {
          var response = JSON.parse(response);
          console.log(response);
          var select = '<select name="users[]" id="users" class="form-control select3" multiple>';
          response.forEach(function (projectUser) {
            console.log(projectUser.first_name);
          var option = '<option value="' + projectUser.id + '">' + projectUser.first_name + ' ' + projectUser.last_name + '</option>';
              select += option;
          });
          select += '</select>';
          $('#users_div').html(select);
          $('.select3').select2();
